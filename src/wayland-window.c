...
createBuffer(const int width, const int height)
{
  ...
  if ((fd = allocateShm(size)) < 0) {
    ...
    wl_buffer_add_listener(
      client_buffer->wl_buffer, &wl_buffer_listener, client_buffer);
...
paintClientSurface(void)
{
  ...
  bufferCopyStretch(renderer_buffer->mem,
                    renderer_buffer->width,
                    renderer_buffer->height,
                    renderer_buffer->stride,
                    client_buffer_mem,
                    client_buffer->width,
...
#define XDG_WM_BASE_VERSION 2
    ...
    xdg_wm_base_add_listener(
      client_objs->xdg_wm_base, &xdg_wm_base_listener, NULL);
  }
}
                      ...
                      uint32_t serial)
{
  ...
  if (client_objs->client_buffer.wl_buffer) {
    wl_surface_attach(
      client_objs->wl_surface, client_objs->client_buffer.wl_buffer, 0, 0);
    wl_surface_damage_buffer(client_objs->wl_surface,
                             0,
                             0,
...
static void
new_frame(void* data, struct wl_callback* wl_callback, uint32_t time)
{
  WlClientObjects* client_objs = KANSO_CAST(WlClientObjects*, data);
  WlClientBuffer* client_buffer = &client_objs->client_buffer;
  ...
  struct wl_callback* frame_callback =
    wl_surface_frame(client_objs->wl_surface);
  wl_callback_add_listener(
    frame_callback, &wl_frame_callback_listener, client_objs);
...
void
wlSetRendererBuffer(void* mem, int width, int height)
{
...
void
wlInitializeWindow(char* name, void* mem, const int width, const int height)
{
  ...
  wl_registry_add_listener(
    client_objs.wl_registry, &wl_registry_listener, &client_objs);
  ...
  xdg_surface_add_listener(
    client_objs.xdg_surface, &xdg_surface_listener, &client_objs);
  client_objs.xdg_toplevel = xdg_surface_get_toplevel(client_objs.xdg_surface);
  xdg_toplevel_add_listener(
    client_objs.xdg_toplevel, &xdg_toplevel_listener, &client_objs);
...
void
wlSetExitCallback(void (*callback_function)(void))
{
