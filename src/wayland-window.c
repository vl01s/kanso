...
static void createBuffer(const int width, const int height) {
  ...
  if ((fd = allocateShm(size)) < 0) {
    ...
    client_buffer->wl_buffer = wl_shm_pool_create_buffer(
        pool, 0, width, height, stride, WL_SHM_FORMAT_XRGB8888);
    wl_buffer_add_listener(client_buffer->wl_buffer, &wl_buffer_listener,
                           client_buffer);
...
paintClientSurface(void)
{
  ...
  bufferCopyStretch(renderer_buffer->mem, renderer_buffer->width,
                    renderer_buffer->height, renderer_buffer->stride,
                    client_buffer_mem, client_buffer->width,
                    client_buffer->height, client_buffer->stride);
...
{
  WlClientObjects *client_objs = KANSO_CAST(WlClientObjects *, data);

  if (strcmp(interface, wl_compositor_interface.name) == 0) {
#define WL_COMPOSITOR_VERSION 5
    client_objs->wl_compositor = wl_registry_bind(
        wl_registry, name, &wl_compositor_interface, WL_COMPOSITOR_VERSION);
  } else if (strcmp(interface, wl_shm_interface.name) == 0) {
#define WL_SHM_VERSION 1
    client_objs->wl_shm =
        wl_registry_bind(wl_registry, name, &wl_shm_interface, WL_SHM_VERSION);
  } else if (strcmp(interface, xdg_wm_base_interface.name) == 0) {
#define XDG_WM_BASE_VERSION 2
    client_objs->xdg_wm_base = wl_registry_bind(
        wl_registry, name, &xdg_wm_base_interface, XDG_WM_BASE_VERSION);
    xdg_wm_base_add_listener(client_objs->xdg_wm_base, &xdg_wm_base_listener,
                             NULL);
  }
}
                                  ...
                                  uint32_t serial)
{
  ...
  if (client_objs->client_buffer.wl_buffer) {
    wl_surface_attach(client_objs->wl_surface,
                      client_objs->client_buffer.wl_buffer, 0, 0);
    wl_surface_damage_buffer(client_objs->wl_surface, 0, 0,
                             client_objs->client_buffer.width,
...
{
  // NOTE(DrKJeff16): deliberately left blank
  // char* wm_capabilities_names[] = { "", "window_menu", "maximize",
  // "fullscreen", "minimize" };
}
static const struct xdg_toplevel_listener xdg_toplevel_listener = {
...
{
  WlClientBuffer *buffer = KANSO_CAST(WlClientBuffer *, data);
...
static void new_frame(void *data, struct wl_callback *wl_callback,
                      uint32_t time)
{
  WlClientObjects *client_objs = KANSO_CAST(WlClientObjects *, data);
  WlClientBuffer *client_buffer = &client_objs->client_buffer;
  ...
  wl_callback_add_listener(frame_callback, &wl_frame_callback_listener,
                           client_objs);
...
void wlInitializeWindow(char *name, void *mem, const int width,
                        const int height)
{
  client_objs.wl_display = wl_display_connect(NULL);
  if (null_ptr(client_objs.wl_display)) {
    verr("(wlInitializeWindow): %s\n", "NULL pointer to `wl_display`");
  }

  client_objs.wl_registry = wl_display_get_registry(client_objs.wl_display);
  if (null_ptr(client_objs.wl_registry)) {
    verr("(wlInitializeWindow): %s\n", "NULL pointer to `wl_registry`");
  }

  wl_registry_add_listener(client_objs.wl_registry, &wl_registry_listener,
                           &client_objs);
  wl_display_roundtrip(client_objs.wl_display);

  client_objs.wl_surface =
      wl_compositor_create_surface(client_objs.wl_compositor);
  ...
  client_objs.xdg_surface = xdg_wm_base_get_xdg_surface(client_objs.xdg_wm_base,
                                                        client_objs.wl_surface);
  xdg_surface_add_listener(client_objs.xdg_surface, &xdg_surface_listener,
                           &client_objs);
  client_objs.xdg_toplevel = xdg_surface_get_toplevel(client_objs.xdg_surface);
  xdg_toplevel_add_listener(client_objs.xdg_toplevel, &xdg_toplevel_listener,
                            &client_objs);
